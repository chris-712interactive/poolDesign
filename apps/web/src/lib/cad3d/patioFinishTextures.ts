...
const JOINT_IN = 0.22; // ~³⁄₁₆″ sand joint — reads at 3D camera distance
const RECTIFIED_JOINT_IN = 0.08; // ~¹⁄₁₆″ rectified porcelain
...
function makePair(
  ...
  const normal = normalFromHeightGray(rctx, size, true, normalStrength);
...
function fillGrout(
  ...
  c.fillStyle = tint(accent, -28);
  c.fillRect(0, 0, s, s);
  // Bright roughness → low height (inverted normals) so joints sit in a dip.
  r.fillStyle = "#f4f4f4";
  r.fillRect(0, 0, s, s);
}

/**
 * Chamfer into the height map so grout reads as a V between units.
 * Color gets a light/shadow lip; roughness is the height field (inverted).
 */
function bevelUnit(
  c: CanvasRenderingContext2D,
  r: CanvasRenderingContext2D,
  ...
  if (w < 4 || h < 4) return;
  if (!beveled) return;
  const b = Math.max(4, Math.min(16, Math.min(w, h) * 0.14));

  const top = c.createLinearGradient(0, y, 0, y + b);
  top.addColorStop(0, "rgba(255,255,255,0.5)");
  top.addColorStop(1, "rgba(255,255,255,0)");
  c.fillStyle = top;
  c.fillRect(x, y, w, b);

  const left = c.createLinearGradient(x, 0, x + b, 0);
  left.addColorStop(0, "rgba(255,255,255,0.32)");
  left.addColorStop(1, "rgba(255,255,255,0)");
  c.fillStyle = left;
  c.fillRect(x, y, b, h);

  const bot = c.createLinearGradient(0, y + h, 0, y + h - b);
  bot.addColorStop(0, "rgba(0,0,0,0.5)");
  bot.addColorStop(1, "rgba(0,0,0,0)");
  c.fillStyle = bot;
  c.fillRect(x, y + h - b, w, b);

  const right = c.createLinearGradient(x + w, 0, x + w - b, 0);
  right.addColorStop(0, "rgba(0,0,0,0.42)");
  ...
  const heightBand = (
    ...
    const g = r.createLinearGradient(gx0, gy0, gx1, gy1);
    g.addColorStop(0, "rgb(242,242,242)");
    g.addColorStop(0.45, "rgba(210,210,210,0.7)");
    g.addColorStop(1, "rgba(40,40,40,0)");
    r.fillStyle = g;
    r.fillRect(rx, ry, rw, rh);
  };
  heightBand(x, y, x, y + b, x, y, w, b);
  heightBand(x, y + h, x, y + h - b, x, y + h - b, w, b);
  heightBand(x, y, x + b, y, x, y, b, h);
  heightBand(x + w, y, x + w - b, y, x + w - b, y, b, h);
}
...
function fillStone(
  ...
  finish: PatioFinish,
  n: number,
  opts?: { mottled?: boolean; cooler?: boolean; beveled?: boolean },
) {
  ...
  r.fillRect(x, y, w, h);
  bevelUnit(c, r, x, y, w, h, opts?.beveled !== false);
}
  ...
  return makePair(
    ...
    (c, r, s) => {
      ...
      for (const [ix, iy, iw, ih] of FRENCH_48_IN) {
        ...
        fillStone(c, r, x, y, w, h, finish, n, {
          mottled,
          beveled: patioFinishBeveled(finish),
        });
      }
    },
    coverageM,
...
function drawStampedCobble(finish: PatioFinish, size: number): PatioTexPair {
  // Packed cobbles ~4–6″ — 24″ module so joints tile cleanly.
    ...
    (c, r, s) => {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          const n = fbm(col * 1.1, row * 1.1, 0xc0bb, 3);
          const jx = (hash2(col, row, 19) - 0.5) * cw * 0.18;
          const jy = (hash2(col, row, 23) - 0.5) * ch * 0.18;
          const w = cw * (0.72 + n * 0.12);
          const h = ch * (0.68 + (1 - n) * 0.14);
          const x = col * cw + (cw - w) / 2 + jx;
          const y = row * ch + (ch - h) / 2 + jy;
          c.beginPath();
          c.ellipse(
            x + w / 2,
            y + h / 2,
          ...
          c.fillStyle = tint(finish.color, (n - 0.5) * 38);
          c.fill();
          r.beginPath();
          r.ellipse(
            x + w / 2,
          ...
          r.fillStyle = `rgb(${55 + n * 50},${55 + n * 50},${55 + n * 50})`;
          r.fill();
          bevelUnit(c, r, x, y, w, h, patioFinishBeveled(finish));
        }
      }
    },
    coverageM,
  ...
  return makePair(
    ...
    (c, r, s) => {
      ...
      for (let row = 0; row < cols; row++) {
        ...
        const h = plankH - 4;
        c.fillStyle = tint(finish.color, (n - 0.5) * 32);
        c.fillRect(0, y, s, h);
        r.fillStyle = `rgb(${90 + n * 50},${90 + n * 50},${90 + n * 50})`;
        r.fillRect(0, y, s, h);
        bevelUnit(c, r, 0, y, s, h, patioFinishBeveled(finish));
        c.strokeStyle = `rgba(${finish.accent.r},${finish.accent.g},${finish.accent.b},0.28)`;
        c.lineWidth = 1;
        for (let k = 0; k < 5; k++) {
          const gy = y + 4 + k * (h / 6);
          c.beginPath();
          c.moveTo(0, gy);
          for (let x = 0; x < s; x += 6) {
            c.lineTo(x, gy + Math.sin(x * 0.08 + row) * 1.2);
          }
          c.stroke();
        }
        const seam = (row * 0.37 + n) % 1;
        c.fillStyle = "rgba(0,0,0,0.22)";
        c.fillRect(s * seam - 1.5, y, 3, h);
      }
    },
    coverageM,
    5.2,
...
function drawRunningBond(finish: PatioFinish, size: number): PatioTexPair {
  ...
  return makePair(
    ...
    (c, r, s) => {
      ...
      const tw = s / cols;
      const th = s / rows;
      const joint = Math.max(1.6, (JOINT_IN / BRICK_W_IN) * tw);
      for (let row = 0; row < rows; row++) {
        ...
        for (let col = -1; col <= cols; col++) {
          ...
          r.fillStyle = `rgb(${48 + n * 36},${48 + n * 36},${48 + n * 36})`;
          r.fillRect(x, y, ww, hh);
          bevelUnit(c, r, x, y, ww, hh, patioFinishBeveled(finish));
        }
      }
    },
    coverageM,
...
function drawHerringbone(finish: PatioFinish, size: number): PatioTexPair {
  ...
  return makePair(
    ...
    (c, r, s) => {
      ...
      const joint = Math.max(1.4, JOINT_IN * ppi);
      for (let row = -2; row < 20; row++) {
        for (let col = -2; col < 20; col++) {
          ...
          r.translate(col * step * 2, row * step * 2);
          const rot = ((col + row) % 2 === 0 ? 1 : -1) * (Math.PI / 4);
          c.rotate(rot);
          r.rotate(rot);
          const x = -bw * 0.5 + joint * 0.5;
          const y = -bh * 0.5 + joint * 0.5;
          const ww = bw - joint;
          const hh = bh - joint;
          c.fillStyle = tint(finish.color, (n - 0.5) * 26);
          c.fillRect(x, y, ww, hh);
          r.fillStyle = `rgb(${52 + n * 40},${52 + n * 40},${52 + n * 40})`;
          r.fillRect(x, y, ww, hh);
          bevelUnit(c, r, x, y, ww, hh, patioFinishBeveled(finish));
          c.restore();
          r.restore();
...
function drawBasketweave(finish: PatioFinish, size: number): PatioTexPair {
  ...
  return makePair(
    ...
    (c, r, s) => {
      fillGrout(c, r, s, finish.accent);
      const cell = s / cells;
      const joint = Math.max(1.4, (JOINT_IN / BRICK_W_IN) * (cell / 2));
        ...
        for (let col = 0; col < cells; col++) {
          ...
          if (horiz) {
            for (let k = 0; k < 2; k++) {
              ...
              r.fillStyle = `rgb(${50 + n * 30},${50 + n * 30},${50 + n * 30})`;
              r.fillRect(
                x0 + joint,
                y0 + joint + k * (cell / 2),
                cell - joint * 2,
      ...
      for (let row = 0; row < count; row++) {
        for (let col = 0; col < count; col++) {
          ...
          fillStone(
            ...
            finish,
            n,
            { mottled: true, beveled: patioFinishBeveled(finish) },
          );
        }
      }
    },
...
function drawPorcelain(finish: PatioFinish, size: number): PatioTexPair {
  ...
  return makePair(
    ...
    (c, r, s) => {
      fillGrout(c, r, s, finish.accent);
      const tw = s / count;
      const joint = Math.max(1.2, (RECTIFIED_JOINT_IN / tileIn) * tw);
      for (let row = 0; row < count; row++) {
        for (let col = 0; col < count; col++) {
...
export function getPatioFinishTexture(finishId: string | undefined): PatioTexPair {
  ...
  const cacheKey = `${finish.id}@v4-bevel-dip`;
